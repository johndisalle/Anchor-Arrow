...
struct CircleDetailView: View {
    ...
    private func blockUser(_ userId: String) async {
        guard let uid = Auth.auth().currentUser?.uid else { return }
        do {
            let updated = (userStore.appUser?.blockedUserIds ?? []) + [userId]
            try await service.updateUser(uid: uid, fields: ["blockedUserIds": updated])
...
struct CommentsSheet: View {
    ...
    private func blockCommentUser(_ userId: String) async {
        guard let uid = Auth.auth().currentUser?.uid else { return }
        do {
            let updated = (userStore.appUser?.blockedUserIds ?? []) + [userId]
            try await service.updateUser(uid: uid, fields: ["blockedUserIds": updated])
